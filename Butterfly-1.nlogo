globals [ q clumping-measure radius ]
patches-own [ elevation used? ]
turtles-own [ start-patch ]

to setup
  ca
  set radius radius.slide
  ask patches
  [
  ...
  [
    ...
    set pcolor scale-color green elevation 0 400  ; set the color of each patch based on its elevation
  ]
  ...
  [
    ...
    pen-down
    set start-patch patch-here ;reports the patch that a turtle is currently on
    set used? TRUE
  ]
...
to setup_real
  ...
  [
    set size 5
    setxy random-pxcor random-pycor ;starting cordinates of the turtle
    set shape "butterfly"
    pen-down
...
to go
  ...
  tick
  if ticks >= 1000
  [let final-corridor-width corridor-width ;Sets a local variable final-corridor-width that we will display at the end
    output-print word "corridor-width "  final-corridor-width
    export-plot "corridor-width"
    ( word "Corridor_output_for_q_"  q  ".csv" ) ; exports the (poorly formatted) plot output as a csv file

    let final-clumping clumping ; Calculate the clumping measure at the end
    output-print word "Final clumping measure: " final-clumping
    stop
  ] ; once the turtle has moved 1000 ticks it stops
end

to move
  ;if elevation >= max [elevation] of neighbors
  ;[stop]

  ifelse random-float 1 < q  ; picks a random decimal between 1 and 0
  ;and if it is less than q then the turtles go to a heigher elevation
  ;patch and if not the turtle move to a neighboring patch
...
;Modify Movement to Select Randomly Among Higher Neighbors
;to move
 ; let higher-neighbors patches with [elevation > [elevation] of myself] ; Get all neighbors that are higher
  ;if any? higher-neighbors [
   ; ifelse random-float 1 < q  ; Pick randomly based on q
    ;[
     ; move-to one-of higher-neighbors ; Move to a randomly selected higher neighbor
    ;]
    ;[
    ;  move-to one-of neighbors ; If not moving uphill, pick a random neighbor
    ;]
  ;]
...
to-report corridor-width
  ...
  report(num-patches-used / mean-path-dist) ; reports (~returns) corridor width

end

to-report clumping
  let total-clumping 0
  ask turtles [
    let num-nearby count turtles in-radius radius
    set total-clumping total-clumping + num-nearby - 1 ; Subtract 1 to exclude the turtle itself
  ]
  report total-clumping / count turtles ; Return the average number of nearby butterflies
end

to clear-traces
  clear-drawing ;erases all the lines drawn by butterfly in "pen-down" mode
end
@#$#@#$#@
GRAPHICS-WINDOW
499
